...
source ${TOP}/device/nexell/allo_display/common.sh
...
revert_common ${TOP}/device/nexell/allo_display/patch
patch_common ${TOP}/device/nexell/allo_display/patch
...
DTB_IMG=${KERNEL_DIR}/arch/arm/boot/dts/s5p4418-allo_display-rev01.dtb


CROSS_COMPILE="arm-eabi-"

if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_BL1}" == "true" ]; then
	build_bl1_s5p4418 ${BL1_DIR}/bl1-${TARGET_SOC} s5p4418 allo_display 0
fi
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_KERNEL}" == "true" ]; then
	build_kernel ${KERNEL_DIR} ${TARGET_SOC} ${BOARD_NAME} s5p4418_allo_display_nougat_defconfig ${CROSS_COMPILE}
	test -d ${OUT_DIR} && \
...
if [ -f ${UBOOT_DIR}/u-boot.bin ]; then
	...
	UBOOT_BOOTARGS="console=ttyAMA3,115200n8 printk.time=1 androidboot.hardware=allo_display androidboot.console=ttyAMA3 androidboot.serialno=s5p4418_allo_display initrd=0x79000000,0x200000"
...
make_ext4_recovery_image \
	...
	${KERNEL_DIR}/arch/arm/boot/dts/s5p4418-allo_display-rev01.dtb \
